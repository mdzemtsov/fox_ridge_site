...
| Arabic production homepage | Passed | After deployment propagation, `https://www.foxridgeequity.com/ar` rendered Arabic hero content, right-to-left navigation and footer, reciprocal EN/简体中文 switcher, and Arabic route links. Client-side title updated to the Arabic SEO title. |
| Arabic production form workflow | Passed | A single approved test submission was accepted at `/ar/contact?interest=research`. The Arabic success state rendered after submission: `شكرًا لك... إذا وُجد توافق، سيتواصل معك أحد أعضاء فريق FoxRidge لترتيب تعريف أولي سري.` The submission used API-compatible English enum values and the `ar` locale path. The internal notification and Arabic applicant acknowledgement were requested from the production handler to `partners@foxridgeequity.com`. |
