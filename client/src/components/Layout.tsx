...
import { getLocale, SiteLocale, toLocalizedPath } from "@/lib/locale";
...
const navigation: Record<SiteLocale, { desktop: NavigationItem[]; mobile: NavigationItem[] }> = {
  en: {
    desktop: [
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const desktopNavigation = navigation[locale].desktop;
  const mobileNavigation = navigation[locale].mobile;
  const localize = (path: string) => toLocalizedPath(path, locale);
  ...
  return (
    <div dir={arabic ? "rtl" : "ltr"} className={cn("min-h-screen flex flex-col font-sans bg-background text-foreground selection:bg-secondary selection:text-white", locale === "zh" && "locale-zh", arabic && "locale-ar")}>
      <a href="#main-content" className={cn("sr-only fixed top-4 z-[60] bg-secondary px-4 py-3 text-sm font-bold text-white shadow-lg focus:not-sr-only focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2", arabic ? "right-4" : "left-4")}>
        {localeCopy.skip}
      </a>
      <header
        className={cn(
          ...
          <nav className="hidden xl:flex min-w-0 items-center justify-end gap-0.5" aria-label={localeCopy.primaryNavigation}>
            {desktopNavigation.map((item) => {
              ...
              return <Link key={item.path} href={localize(item.path)} aria-current={active ? "page" : undefined} className={cn("relative whitespace-nowrap rounded-sm px-2.5 py-2 text-[11px] font-semibold tracking-[0.01em] transition-colors duration-200 2xl:px-3 2xl:text-xs", active ? "bg-primary/5 text-primary" : "text-muted-foreground hover:bg-primary/[0.035] hover:text-primary")}>
                {item.name}
                <span className={cn("absolute inset-x-2.5 bottom-0 h-[2px] scale-x-0 bg-secondary transition-transform duration-200 2xl:inset-x-3", arabic ? "origin-right" : "origin-left", active && "scale-x-100")} />
              </Link>;
            })}
            ...
            <Link href={localize("/contact")} className="ms-2.5 shrink-0"><Button className="h-10 bg-secondary px-3.5 text-[10px] font-bold uppercase tracking-[0.08em] text-white hover:bg-primary 2xl:px-4">{localeCopy.introduction}<ArrowUpRight className={cn("ms-2 h-3.5 w-3.5", arabic && "-scale-x-100")} aria-hidden="true" /></Button></Link>
          </nav>

          <div className="flex items-center gap-1 xl:hidden">
            {alternateLanguages.map((alternate) => <Link key={alternate} href={toLocalizedPath(location.split("?")[0] || "/", alternate)} className="rounded-sm px-2 py-2 text-[11px] font-bold text-primary transition-colors hover:text-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-secondary focus-visible:ring-offset-2" aria-label={languageAriaLabels[alternate]}>{alternate === "zh" ? "中文" : languageLabels[alternate]}</Link>)}
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              ...
              <SheetContent side={arabic ? "left" : "right"} className={cn("w-[min(100vw,430px)] p-0", arabic ? "border-r" : "border-l")}>
                <div className="flex h-full flex-col bg-background" dir={arabic ? "rtl" : "ltr"}>
                  ...
                  <nav className="flex flex-1 flex-col gap-1 overflow-y-auto px-5 py-5" aria-label={localeCopy.mobileNavigation}>
                    {mobileNavigation.map((item, index) => {
                      ...
                      return <Link key={item.path} href={localize(item.path)} aria-current={active ? "page" : undefined} onClick={() => setIsOpen(false)} className={cn("group flex min-h-14 items-center justify-between px-4 py-3 text-lg font-display font-bold transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-secondary focus-visible:ring-offset-2", arabic ? "border-r-2" : "border-l-2", active ? "border-secondary bg-primary/[0.045] text-primary" : "border-transparent text-muted-foreground hover:border-secondary/50 hover:bg-primary/[0.035] hover:text-primary")}><span className="flex items-center gap-3"><span className="text-[10px] font-mono font-medium text-secondary/80">0{index + 1}</span>{item.name}</span><Chevron className={cn("h-4 w-4 text-secondary transition-transform", active ? "translate-x-0" : arabic ? "translate-x-1 opacity-0 group-hover:translate-x-0 group-hover:opacity-100" : "-translate-x-1 opacity-0 group-hover:translate-x-0 group-hover:opacity-100")} aria-hidden="true" /></Link>;
                    })}
                  </nav>
                  <div className="border-t border-border bg-stone-50 px-5 py-5"><Link href={localize("/contact")} onClick={() => setIsOpen(false)}><Button className="min-h-14 w-full whitespace-normal bg-secondary px-5 py-3 text-sm font-bold leading-snug text-white hover:bg-primary">{localeCopy.introduction}<ArrowUpRight className={cn("ms-2 h-4 w-4 shrink-0", arabic && "-scale-x-100")} aria-hidden="true" /></Button></Link></div>
                </div>
              </SheetContent>
            </Sheet>
          </div>
